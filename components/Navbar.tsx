      ...
      <nav className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-5 text-black">
          {session?.user ? ( // If user is logged in, show these buttons (if session exists and if that session has a user)
            ...
            <form
              action={async () => {
                // You must specify "use server" or else Next.js will assume that the function should run in the browser (client-side). signIn and signOut are SERVER-ONLY FUNCTIONS
                "use server";
                await signIn("github");
              }}
            >
